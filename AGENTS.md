...
**Live ICP comes from the brief** (Settings in the CRM). The industry /
titles / regions section below is the *owner's current profile* for this
repo — when a brief is pasted into your task, prefer the brief's ICP over
any stale copy here. Friends using this CRM for other markets will have
different Settings; their briefs are the source of truth.
...
The CRM also ships a **local enricher** (`python -m crm enrich` / **Run
enricher** in the UI) that does public web search + page scrape with no paid
APIs. When that covers a prospect, you may not be asked. When you *are* given
a Stage 2 brief, treat it as the harder residual research the local engine
could not finish.
...
| `photo_url` | wanted | Public headshot image URL (CRM downloads a local copy) |
...
- **Narrow every search** to the three anchors in the brief: **full name +
  company + city**. Do not chase same-name people at other companies or in
  other cities. If city is missing, verify it first (company site / HQ /
  office pages / listed region), write it into `city`, then lock personal
  searches to that city. Company-only news is fine for rapport `notes`;
  personal contact data must match name + company (and city when known).
