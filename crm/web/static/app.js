...
(function () {
  ...
  // ---- enrich multi-select (prospects list) ------------------------------
  var ENRICH_CAP = 10;
  var enrichSelected = {}; // id -> name
  var enrichBar = document.getElementById("enrich-bar");
  var enrichCount = document.getElementById("enrich-count");
  var enrichIds = document.getElementById("enrich-ids");
  var enrichSubmit = document.getElementById("enrich-submit");
  var enrichBriefSubmit = document.getElementById("enrich-brief-submit");
  var enrichClear = document.getElementById("enrich-clear");

  function enrichSelectionSize() {
    return Object.keys(enrichSelected).length;
  }

  function fillIdInputs(container) {
    if (!container) return;
    container.innerHTML = "";
    Object.keys(enrichSelected).forEach(function (id) {
  ...
  function renderEnrichBar() {
    if (!enrichBar) return;
    var n = enrichSelectionSize();
    enrichBar.hidden = n === 0;
    if (enrichCount) {
      enrichCount.textContent = n === 1 ? "1 selected" : n + " selected";
    }
    fillIdInputs(enrichIds);
    fillIdInputs(document.querySelector(".enrich-ids-brief"));
    if (enrichSubmit) enrichSubmit.disabled = n === 0;
    if (enrichBriefSubmit) enrichBriefSubmit.disabled = n === 0;
  }

  function syncEnrichPicks() {
    document.querySelectorAll(".enrich-pick").forEach(function (box) {
      box.checked = !!enrichSelected[box.value];
    });
    var pageBoxes = document.querySelectorAll(".enrich-pick");
    var selectPage = document.getElementById("select-page");
    if (selectPage && pageBoxes.length) {
      var checked = 0;
      pageBoxes.forEach(function (box) { if (box.checked) checked += 1; });
      selectPage.checked = checked > 0 && checked === pageBoxes.length;
      selectPage.indeterminate = checked > 0 && checked < pageBoxes.length;
    } else if (selectPage) {
  ...
  document.addEventListener("change", function (event) {
    ...
    if (target && target.classList && target.classList.contains("enrich-pick")) {
      if (target.checked) {
        if (enrichSelectionSize() >= ENRICH_CAP && !enrichSelected[target.value]) {
          target.checked = false;
          toast("Enrich up to " + ENRICH_CAP + " at a time — clear some first", true);
          return;
        }
        enrichSelected[target.value] = target.getAttribute("data-name") || target.value;
      } else {
        delete enrichSelected[target.value];
      }
      syncEnrichPicks();
      return;
    }
    if (target && target.id === "select-page") {
      var boxes = document.querySelectorAll(".enrich-pick");
      if (target.checked) {
        boxes.forEach(function (box) {
          if (enrichSelectionSize() >= ENRICH_CAP && !enrichSelected[box.value]) return;
          enrichSelected[box.value] = box.getAttribute("data-name") || box.value;
          box.checked = true;
        });
        if (enrichSelectionSize() >= ENRICH_CAP) {
          toast("Capped at " + ENRICH_CAP + " prospects for one enrich job");
        }
      } else {
        boxes.forEach(function (box) {
          delete enrichSelected[box.value];
          box.checked = false;
        });
      }
      syncEnrichPicks();
  ...
  if (enrichClear) {
    enrichClear.addEventListener("click", function () {
      enrichSelected = {};
      syncEnrichPicks();
    });
  }

  function armEnrichRunForm(form) {
    if (!form) return;
    form.addEventListener("submit", function () {
      ...
      toast("Enricher is searching the public web — keep this tab open (30–90 seconds)");
    });
  }
  armEnrichRunForm(document.getElementById("enrich-selected-form"));
  document.querySelectorAll("form.enrich-run-form").forEach(armEnrichRunForm);
