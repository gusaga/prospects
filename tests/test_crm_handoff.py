...
def test_brief_contains_icp_and_exclusions():
    from crm.web.routes_data import EXAMPLE_ICP, build_brief


    brief = build_brief(EXAMPLE_ICP, ["known.example", "other.example"])
...
def test_enrich_brief_lists_targets_and_priorities(session):
    ...
    from crm.web.routes_data import EXAMPLE_ICP, build_enrich_brief


    ingest_records(session, [record()], filename="a.json", source="codex")
    target = session.scalar(select(Prospect))
    req = ResearchRequest(kind="enrich", target_ids=[target.id], requested_count=1)
    brief = build_enrich_brief(EXAMPLE_ICP, req, [target])
    ...
    assert "SEARCH ANCHORS" in brief
    assert "NARROW THE SEARCH" in brief
    assert 'Full name: "Jane Doe"' in brief
...
def test_discovery_brief_learns_from_decisions():
    ...
    brief = build_brief(EXAMPLE_ICP, [], decisions=decisions)
