...
EDITABLE_PROSPECT_FIELDS = {
    "full_name", "title", "phone", "email", "linkedin_url", "region",
...
def prospect_query(session: Session, f: dict):
    ...
    if f["q"]:
        ...
        stmt = stmt.where(or_(
            ...
            Prospect.region.ilike(like), Company.name.ilike(like),
            Company.domain.ilike(like), Company.industry.ilike(like),
            Company.region.ilike(like),
        ))
    if f["status"]:
...
def create_app() -> FastAPI:
    ...
    def today_view(request: Request, session: Session = Depends(get_session)):
        ...
        return render("today.html", request, due=due, queued=queued, called_today=called_today)
    ...
    def prospect_list(request: Request, session: Session = Depends(get_session)):
        ...
        context = dict(prospects=prospects, f=f, regions=region_options(session),
                       count=len(prospects), query_string=request.url.query)
